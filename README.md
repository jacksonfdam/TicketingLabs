...
- [~] **Phase 3 — The other backends**, each passing the same contract tests.
      **FastAPI**, **NestJS**, **Express**, and **Laravel** are done (same 16 tests,
      zero changes to the suite or frontend). Remaining: Symfony, Phalcon.
...
`fastapi`, `nest`, `express`, or `laravel`) and running `make up` again. The frontend,
gateway, and contract do not change. See
