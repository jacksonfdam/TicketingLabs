...
// atomic within the single-threaded event loop, and the Locker serialises per key, so
// the reservation tests exercise the real orchestration.

import { Errors } from '../domain/errors';
import { Event, Order, Payment, QueueToken, Reservation, ReservationStatus, Sector, User } from '../domain/models';
import {
  EventRepository,
  LockHandle,
  Locker,
...
export class MemEvents implements EventRepository {
  constructor(private s: Store) {}
  async list(_c: string, limit: number) {
