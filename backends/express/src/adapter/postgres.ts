// Postgres adapter over node-postgres (pg). The star is SectorsRepo.decrementInventory:
// one conditional UPDATE that makes overselling impossible at the database.
...
// Lenient UUID shape check (8-4-4-4-12 hex). Deliberately NOT the uuid package's
// validate(), which enforces RFC 4122 version/variant bits and so rejects the lab's
// tidy seed ids like 1111...1111 that Postgres accepts. Only rejects genuine garbage
// (which would otherwise 500 the DB with an invalid-uuid error).
...
export class EventsRepo implements EventRepository {
  ...
  async list(cursor: string, limit: number) {
    ...
    if (!cursor) {
      ...
      if (!isUuid(cursor)) throw Errors.BadRequest; // malformed cursor is a 400, not a 500
      ({ rows } = await this.pool.query(`SELECT ${cols} FROM events WHERE id > $1 ORDER BY id LIMIT $2`, [cursor, limit + 1]));
    }
...
export class SectorsRepo implements SectorRepository {
  ...
  async incrementInventory(sectorId: string, qty: number) {
    await this.pool.query('UPDATE sectors SET available_inventory = available_inventory + $2 WHERE id = $1', [sectorId, qty]);
  }
}
...
export class QueueRepo implements QueueRepository {
  ...
  async find(userId: string, eventId: string) {
    ...
    const r = rows[0];
    if (!r) return null;
    return { id: r.id, userId: r.user_id, eventId: r.event_id, position: r.position, status: r.status as QueueStatus, admittedAt: r.admitted_at };
  }
  async nextPosition(eventId: string) {
    const { rows } = await this.pool.query('SELECT COALESCE(MAX(position)+1, 0) AS pos FROM queue_tokens WHERE event_id = $1', [eventId]);
...
export class ReservationsRepo implements ReservationRepository {
  constructor(private pool: Pool) {}
  private static COLS = 'id, user_id, sector_id, quantity, status, expires_at, idempotency_key, created_at';
...
export class OrdersRepo implements OrderRepository {
  ...
  async findByReservationId(reservationId: string) {
    const { rows } = await this.pool.query(`SELECT ${OrdersRepo.COLS} FROM orders WHERE reservation_id = $1`, [reservationId]);
    return rows[0] ? this.row(rows[0]) : null;
  }
  async findByIdempotencyKey(userId: string, key: string) {
    const { rows } = await this.pool.query(
...
export class PaymentsRepo implements PaymentRepository {
  ...
  async findByOrderId(orderId: string) {
    ...
    const r = rows[0];
    if (!r) return null;
    return { id: r.id, orderId: r.order_id, providerRef: r.provider_ref, status: r.status as PaymentStatus, attempts: r.attempts };
  }
}
