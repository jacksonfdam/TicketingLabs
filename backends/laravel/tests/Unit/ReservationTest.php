...
// (PHP is single-threaded); they pin the exhaustion, idempotency, release, and sweep
// logic. The concurrency/overselling proof for this backend is the load test.
...
use App\Testing\AllowAllRateLimiter;
...
final class ReservationTest extends TestCase
{
    private function fixture(int $available, $admission = null): array
    {
        ...
        $svc = new ReservationService(
            new MemReservationRepository($store),
            new MemSectorRepository($store),
            new NoopLocker(),
            $admission ?? new AlwaysAdmit(),
    ...
    public function test_sweeper_expires_held(): void
    {
        ...
        $svc = new ReservationService(
            new MemReservationRepository($store), new MemSectorRepository($store),
            new NoopLocker(), new AlwaysAdmit(), $clock, new SeqIds(), 60,
        );
