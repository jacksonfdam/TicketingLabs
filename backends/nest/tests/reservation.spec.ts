// Unit tests for the reservation use case. No Postgres, no Redis, no NestJS. They
// mirror the Go and FastAPI tests, because the same invariants must hold everywhere.
...
import {
  ...
  MemSectors,
  Store,
} from '../src/adapter/memory';
import { Errors } from '../src/domain/errors';
import { EventStatus, Sector } from '../src/domain/models';
import { ReservationService } from '../src/usecase/reservation.service';
...
describe('reservation invariants', () => {
  it('never oversells under concurrency', async () => {
    ...
    const exhausted = results.filter((r) => r === Errors.InventoryExhausted.code).length;

    expect(success).toBe(available);
    expect(exhausted).toBe(buyers - available);
  ...
  it('sweeps expired holds and returns their stock', async () => {
    ...
    const swept = await svc.sweepExpired(100);
    expect(swept).toBe(1);
    expect(store.sectors.get('sec')!.availableInventory).toBe(10);
  });
});
