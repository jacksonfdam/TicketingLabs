...
// Persists refresh tokens so they can be revoked on use. consume must be atomic:
// it returns the owner and removes the token in one step (Redis GETDEL).
export interface RefreshStore {
  save(jti: string, userId: string, ttlSeconds: number): Promise<void>;
  consume(jti: string): Promise<string | null>;
}

// Short-lived access JWTs plus opaque, rotating refresh tokens. Rotating a refresh
// token revokes it, so a stolen-and-replayed refresh token fails. Same design as the
// Go and FastAPI backends.
export class JwtTokenService implements TokenService {
  ...
  issueAccess(userId: string, role: Role): { token: string; expiresIn: number } {
    ...
    const token = jwt.sign(
      { sub: userId, role, iat: now, exp: now + this.accessTtlSeconds },
      this.secret,
      { algorithm: 'HS256' },
    );
  ...
  async rotate(refreshToken: string): Promise<string> {
    ...
    if (!userId) throw Errors.InvalidToken; // unknown, expired, or already spent
    return userId;
  }
