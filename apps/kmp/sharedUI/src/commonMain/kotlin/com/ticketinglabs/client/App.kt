...
import com.ticketinglabs.client.di.demoBackend
import com.ticketinglabs.client.di.realBackend
...
import com.ticketinglabs.client.presentation.toUiState
import com.ticketinglabs.client.theme.AppTheme
import com.ticketinglabs.client.ui.LoginScreen
...
/**
 * The app shell: seven screens, one linear flow, plus a login gate. The composition root — it
 * builds the dependency graph via [demoBackend]/[realBackend] (chosen by [AppConfig.USE_REAL_BACKEND]),
 * gates the flow behind sign-in when a real session is present, and drives navigation. This
 * composable lives in commonMain; the Android ([AppActivity]) and iOS ([MainViewController])
 * entry points both call [App].
 */
@Composable
fun App(onThemeChanged: @Composable (isDark: Boolean) -> Unit = {}) {
    AppTheme(onThemeChanged) {
        ...
        val backend = remember {
            if (AppConfig.USE_REAL_BACKEND) realBackend(ApiConfig(AppConfig.DEFAULT_BASE_URL)) else demoBackend()
        }
        ...
        val eventsVm = viewModel { EventsViewModel(backend.events) }
        val waitingVm = viewModel { WaitingRoomViewModel(backend.queue, pollIntervalMs = 800) }
        ...
        LaunchedEffect(loggedIn) { if (loggedIn) eventsVm.load() }
        ...
        suspend fun loadDetail(id: EventId) {
            detail = when (val r = backend.events.getEvent(id)) {
        ...
        fun submitLogin(email: String, password: String) {
            val session = backend.session ?: return
            loginState = UiState.Loading
            scope.launch {
                loginState = when (val r = session.login(email, password)) {
                    is Outcome.Success -> { loggedIn = true; UiState.Idle }
                    is Outcome.Failure -> r.error.toUiState()
                }
            }
        }
        ...
        Column(Modifier.fillMaxSize()) {
            ...
            Box(Modifier.weight(1f)) {
                if (!loggedIn) {
                    LoginScreen(loginState, onSubmit = ::submitLogin)
                } else {
                    when (screen) {
