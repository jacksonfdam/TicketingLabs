...
class HttpRepositoriesTest {
    ...
    // The Ktor client installs HttpTimeout, whose timeout is a real `delay`. Under runTest's
    // virtual clock, auto-advance would fire that delay before MockEngine responds and spoof a
    // timeout. Running the client body on a real dispatcher keeps the timeout in real time, so
    // the instant mock response wins. These tests assert parsing/mapping, not timing.
    private fun httpTest(body: suspend () -> Unit) = runTest { withContext(Dispatchers.Default) { body() } }

    @Test
    fun events_list_parses_and_ignores_unknown_fields() = httpTest {
    ...
    @Test
    fun an_unknown_enum_value_becomes_MalformedResponse() = httpTest {
        val body = """
    ...
    @Test
    fun an_unparseable_timestamp_becomes_MalformedResponse() = httpTest {
        val body = """
    ...
    @Test
    fun garbage_body_becomes_MalformedResponse() = httpTest {
        val api = Api { respond("{ not json", HttpStatusCode.OK) }
        assertIs<AppError.MalformedResponse>((api.events.getEvent(EventId("e1")) as Outcome.Failure).error)
    }

    @Test
    fun a_409_with_a_backend_code_maps_to_Conflict_and_keeps_the_request_id() = httpTest {
    ...
    @Test
    fun reservation_create_sends_the_idempotency_key_and_body() = httpTest {
    ...
    @Test
    fun a_401_maps_to_Unauthorized() = httpTest {
